...
class AppStore extends Component {
  ...
  filteredResults = searchedApps => {
    const {isAppStore} = this.state
    const filteredProjects = searchedApps.filter(
      eachAppDetails => eachAppDetails.category === isAppStore,
    )
  ...
  render() {
    const {searchInput, isAppStore} = this.state
    const searchResults = this.searchResults()
    const filteredResults = this.filteredResults(searchResults)
    return (
      <div>
        <h1> App Store </h1>
        <input
        ...
        <ul>
          {tabsList.map(eachTab => (
            <TabItem
              ...
              isActive={eachTab.tabId === isAppStore}
            />
          ))}
        </ul>
        <ul>
